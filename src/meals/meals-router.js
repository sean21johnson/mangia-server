...
mealsRouter
	...
	.get(requireAuth, (req, res, next) => {
		...
		if ((filter === "" || !filter) && (search === "" || !search)) {
			...
			MealsService.getSpecificCategory(
				req.app.get("db"),
				filter,
				req.user.users_id
			)
	...
	.post(requireAuth, bodyParser, (req, res, next) => {
		...
		MealsService.insertNewMeal(req.app.get("db"), newMeal)
			.then((meal) => {
				...
				res.status(201).location(`/meals/${newMeal.meal_id}`).json(meal);
			})
			.catch(next);
	});
...
mealsRouter
	...
	.patch(bodyParser, (req, res, next) => {
		...
		MealsService.updateSpecificMeal(
			req.app.get("db"),
			req.params.id,
			mealToUpdate
		)
			.then((meal) => {
				return res.status(200).json(meal).end();
			})
			.catch(next);
	});
